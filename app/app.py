    ...
    if st.button("Predict Churn Risk", type="primary", use_container_width=True):
        ...
        if proba >= 0.5:
            st.warning(
                ...
                "Common retention levers: move to a longer contract, add security/support "
                "add-ons, or switch payment method away from electronic check."
            )
        else:
            st.success("This customer profile looks stable based on historical patterns.")

# ----------------------------- Model Insights tab -----------------------------
with tab_insights:
    st.subheader("What drives churn? (Top 15 features)")
    ...
    fig.update_layout(height=520, showlegend=False, yaxis_title="", xaxis_title="Importance")
    st.plotly_chart(fig, use_container_width=True)

    st.subheader("Model comparison")
    results_df = pd.DataFrame(results["results"]).T
    results_df.index.name = "model"
    st.dataframe(results_df.style.highlight_max(axis=0, color="#d4f7d4"), use_container_width=True)
